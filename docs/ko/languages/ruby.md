...
| 요소 | Kind | XML Tag | 예시 |
|------|------|---------|------|
| 메서드 | `method` | `<function>` | `def greet(name)` |
| 클래스 메서드 | `method` | `<function>` | `def self.create(attrs)` |
| 클래스 | `class` | `<type>` | `class User < ActiveRecord::Base` |
| 모듈 | `namespace` | `<type>` | `module Authentication` |
| 상수 (top-level) | `variable` | `<variable>` | `MAX_RETRIES = 3` |
| YARD 주석 | `doc` | | `# 설명` |
...
- 인스턴스 메서드(`def foo`)와 클래스 메서드(`def self.foo`) 모두 kind `method`를 사용합니다
